...
var reg_fio = /^[a-zA-Zа-яА-Я-_Ёё]+$/;   // только буквы и дефис - кирилица, латынь
...
if(far_epo("tel_2")) if(!reg_phone.test(polto_rth("tel_2")) && polto_rth("tel_2")) po_tx+=erp("tel_2")+rdl;		
...
if(far_epo("birthday")) if(!polto_rth("birthday")) po_tx+=erp("birthday")+rdl;		
...
// проверка поля Участие в форуме
		...
		if(fop==hifop) {
			...
			var chk=$("#chk").val();   //№ ЧК  
                ...
                success: function(html){ 
					...
					$(".but_preloader").css("display", "none");
                } 
