...
//echo"<pre>";print_r($ar_Result["STATUS_ID"]);echo"</pre>";			 
					...
					// присваиваем Фамилия
					$ar_xml_var["ParticipantLastName"]=par_text(trim($ar_Ans['family'][0]["USER_TEXT"]));
					
					// присваиваем Имя
					$ar_xml_var["ParticipantFirstName"]=par_text(trim($ar_Ans['name'][0]["USER_TEXT"]));
					
					// присваиваем Отчество
					$ar_xml_var["ParticipantMiddleName"]=par_text(trim($ar_Ans['middle_name'][0]["USER_TEXT"]));
					
					// присваиваем Скайп
					$ar_xml_var["SkypeName"]=par_text(trim($ar_Ans['skype'][0]["USER_TEXT"]));
					
					// присваиваем Телефон
					$ar_xml_var["TelephoneNumber"]=par_tel(trim($ar_Ans['tel'][0]["USER_TEXT"]));
					...
					$ar_xml_var["Comment"]=par_text(trim($ar_Ans['comments'][0]["USER_TEXT"]));
					...
					else $errors_request[]=$code_m.": заявка № ".$_GET['RESULT_ID']." Ошибка: Нет или ошибка формата даты поступления заявки(ClaimDate - claimdate)";// если дата поступления заявки не указана или неформат запишем ошибку в массив
...
//echo "<br/>".$ar_Ans['key_edit']['0']['ANSWER_ID'];
						...
						CFormResult::SetField($ar_xml_var["Id"], "key_edit", array ($ar_Ans['key_edit']['0']['ANSWER_ID']=>"2")); //Изменение курса
							}
					}
