...
if(CModule::IncludeModule("form")){ 
	...
	$ar_Answer = CFormResult::GetDataByID(
		$RESULT_ID, 
		array('name','family','email','email_2','em_bd','money_2','currency','history_status','oplata','lang_id','expired','code_m'),  // массив символьных кодов необходимых вопросов
		$ar_Result, 
		$ar_Answer2);  
	...
	if(!$ar_Answer['expired'][0]['USER_TEXT']) CFormResult::SetField( $RESULT_ID, "expired", array ($ar_Answer['expired'][0]['ANSWER_ID'] => (date("d.m.Y"))."-".$PREV_STATUS_ID)); //внесём дату изменения статуса
...
if($CURRENT_STATUS_ID <>$PREV_STATUS_ID) {
	...
	@include $_SERVER["DOCUMENT_ROOT"]."/com/registration_event/".$code_m."/mail_status/lang/".$lang_id."/status_nopl.php"; //шаблон письма на языке регистрации заявки
