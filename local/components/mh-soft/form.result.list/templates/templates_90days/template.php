...
<table class="result_table">
	...
	<? //echo "<pre>"; print_r($arResult["RESULT_KEY_ID"]["QUESTION"]["foto_1"]["TEXT"]); echo "</pre>"?>
	
		<?foreach($arResult["RESULT_KEY_ID"] as $key => $res){?>
<? //echo "<pre>"; print_r($arResult["RESULT_KEY_ID"][QUESTION]); echo "</pre>";?>
<? //echo $res[QUESTION][foto_1][TEXT]; ?>
		<?// echo print_r($key);?>
		<?// if ($res["QUESTION"]["foto_1"]["TEXT"] != "") {?>
		<tr class="tr_res">
				...
				<td><div class="div_act">
					<div class="c_menu">
						<?if($arParams["ENABLE_RESULT_VIEW"]=="Y" AND $arParams["VIEW_URL"]!="") {?>
							<div class='div_c_menu res_view'><div><a href='<?=$arParams["VIEW_URL"]?>?WEB_FORM_ID=<?=$arParams["WEB_FORM_ID"]?>'><span><?=GetMessage("FORM_TABLE_VIEW")?></span></a></div></div>
						<?}?>
			...
			<?}?>
				...
				<?foreach($res["QUESTION"] as $k=> $val){?>
					 <td title='#<?=$key?>: <?=$val["TITLE"]?>'><?=$val["TEXT"]?>
					 <? if (($val["TITLE"]=="Фото до 1" || $val["TITLE"]=="Фото до 2" || $val["TITLE"]=="Фото до 3" || $val["TITLE"]=="Фото 4" || $val["TITLE"]=="Фото 5" || $val["TITLE"]=="Фото 6" || $val["TITLE"]=="Фото 7" || $val["TITLE"]=="Фото 8" || $val["TITLE"]=="Фото 9" || $val["TITLE"]=="Фото 10") && file_exists(str_replace("/ru/smart/90days/","",$val["TEXT"]))) {?> <a href="<?=$val["TEXT"];?>" target="_blank"><img src="<?=str_replace("/ru/smart/90days/","",$val["TEXT"])?>" height="200"></a>  <?}?>
