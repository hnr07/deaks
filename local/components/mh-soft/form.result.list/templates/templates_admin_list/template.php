				...
				<td><div class="div_act">
					<div class="c_menu">
						...
						<?if($arParams["ENABLE_RESULT_COPY"]=="Y") {?>
							<div class='div_c_menu'><div onclick="f_copy(<?=$key?>,<?=$arParams["WEB_FORM_ID"]?>,<?=$res["STATUS_RESULT"]["STATUS_ID"]?>)"><?=GetMessage("FORM_TABLE_COPY")?></div></div>
						<?}?>
						<?if($arParams["ENABLE_RESULT_DEL"]=="Y") {?>
							<div class='div_c_menu'><div onclick="f_del(<?=$key?>)"><?=GetMessage("FORM_TABLE_DEL")?></div></div>
						<?}?>
...
<?if(isset($_REQUEST["copy_new"])) {?>
	<div class='inf_copy_result'><?=GetMessage("RESULT_CREATED", Array ("#ID_NEW_RESULT#" => $_REQUEST["copy_new"]))?></div>
<?}?>
<?if(isset($_REQUEST["copy_error"])) {?>
	<div class='inf_copy_result'><?=GetMessage("ERROR_CREATED")?> <?=$_REQUEST["copy_old"]?></div>
<?}?>
<div class="box_copy"><img class="close" id="close" src="<?=$templateFolder?>/images/close.png">

	<form id="" action="<?=($arParams["COPY_URL"]!="")?$arParams["COPY_URL"]:$templateFolder."/copy_result.php"?>" name="" method="GET">
...
<?if(isset($_REQUEST["but_del"])) {?>
	<div class='inf_copy_result'><?=GetMessage("RESULT_DEL", Array ("#ID_DEL_RESULT#" => $_REQUEST["del_id"]))?></div>
<?}?>
