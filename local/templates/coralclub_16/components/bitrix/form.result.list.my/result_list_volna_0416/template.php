...
$ar_Answer = CFormResult::GetDataByID(
	$arRes['ID'], 
	array('venue','id_venue','chk','family','name','kem_priglashen_chk','kem_priglashen_family','kem_priglashen_name','sum_debt','money_2_calc','currency','promotion_1','fly_1','fly_2','hotel','id_hotel','nomer','id_nomer','p_hotel','d_leader_ship','s_leader_ship','hotel_ls','nomer_ls','status','comments_admin','mesto','mesto_index','id_venue','venue'), 
	$ar_Result, 
	$ar_Answer2);
...
//	include "methods/filter_chk.php"; // Подключаем проверки ЧК и гостей ЧК
	...
	<?	
		...
		foreach($errors as $key=>$val) {
				$str_error.=$val." ";
				echo "<div class='mess_rey'><div><i><b>№".$arRes['ID'].":</b> ".ft('GER_TIT_2')."</i></div>".$val."</div>"; // Текст ошибки
...
$fu=1;
	...
	if($fu) { // Проверка пройдена
		...
		if($choice_place && $ar_Answer['mesto_index'][0]['USER_TEXT']=="") {
			CFormResult::SetStatus($arRes['ID'], $status_rz,"Y");   //Если подключен блок выбора мест в зале и индекс места не задан в статус Резерв
			$rsStatus = CFormStatus::GetByID($status_rz);
			$arStatus = $rsStatus->Fetch();
			$arRes["STATUS_TITLE"]=$arStatus["TITLE"];
			$arRes["STATUS_CSS"]=$arStatus["CSS"];
		...
		////////////////// Блок отправки заявки в базу ////////////////////////		
			include "methods/request.php";	
		///////////////	Конец блока отправки заявки в базу /////////////////////
	}
	else {
		/*
	...
	$mesto=$ar_Answer['mesto'][0]['USER_TEXT'];
	$id_venue=$ar_Answer['id_venue'][0]['USER_TEXT'];
	$venue=$ar_Answer['venue'][0]['USER_TEXT'];
...
<div class="bued">
<?$ar_noc = array();?>
<div class="indu"  <?if(!in_array($id_venue, $ar_noc)) {?>onMouseOver="f_tc(1,'<?=$arRes['ID']?>')" onMouseOut="f_tc(0,'<?=$arRes['ID']?>'<?}?>)"><div><?

if(in_array($id_venue, $ar_noc)) {
	echo "стоимость уточняется";
}
else {
	echo GetMessage('ZDPM')?> - <?echo $sum_debt." ".$ar_Answer['currency'][0]['USER_TEXT'];
}
...
<?if($money_2_calc):?>
	<div class="text_calc" id="text_calc_<?=$arRes['ID']?>"><div><?echo $venue?><br /><br /><?echo $money_2_calc?></div><br/>
		<?if($fly_1 || $fly_2){?>
			<div><?echo GetMessage('FLY').":<br/>".$fly_1."<br/>".$fly_2;?></div>
		<?}?>
		<?if($hotel || $nomer || $hotel_ls || $nomer_ls){?>
			<div><?echo GetMessage('HOTEL').":<br/>".$hotel."<br/>".$nomer."<br/>".$hotel_ls."<br/>".$nomer_ls;?></div>
		<?}?>
		<?if($mesto){?>
			<div><?echo GetMessage('mesto').":<br/>".$mesto;?></div>
		<?}?>
	</div>
<?endif?>
