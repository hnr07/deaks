...
//var reg_mail = /^[a-zA-Z0-9][-\._a-zA-Z0-9]+@(?:[a-zA-Z0-9][-a-zA-Z0-9]+\.)+[a-zA-Z]{2,6}$/; // електронная почта 
    ...
    var reg_dm = /^[0-9]{1,3}$/;   //  число - только 1-3 цифры
...
// проверка поля Перелет туда
    if (document.getElementById('ti_blo_p_fly_1')) {
        var fop = $("#ti_blo_p_fly input:radio").filter(":checked").val();
        var hifop = $("#ti_blo_p_fly input:hidden:eq(0)").val();
        var fl = $("#ti_blo_p_fly_1 input:radio").filter(":checked").val();
        if (fop == hifop) {
            if (fl && fl !== undefined) {
                oker("fly_1", 1);
            }
...
// проверка поля Перелет обратно
    if (document.getElementById('ti_blo_p_fly_2')) {
        var fop = $("#ti_blo_p_fly input:radio").filter(":checked").val();
        var hifop = $("#ti_blo_p_fly input:hidden:eq(0)").val();
        var fl = $("#ti_blo_p_fly_2 input:radio").filter(":checked").val();
...
$("#ti_blo_d_eat_1 input:radio").attr ("checked", false);
    ...
    if (fop == hifop) {
        ...
        $("#ti_blo_p_fly_1 input:radio").filter(":checked").attr("checked", false);
        $("#ti_blo_p_fly_2 input:radio").filter(":checked").attr("checked", false);
...
//////////////////////////////
    ...
    var ard = d.split("-");
    var kld = 0;
    if (dt_s && dt_f && dt_s == ard[0] && dt_f == ard[1]) kld = 1;
   ...
   // var dt_f = $("#ti_blo_day_hotel_finish_ls input:text:eq(0)").val();
    ...
    if (kld) $("#p_fly_0").css("display", "block");
    else {
    ...
    var fop = $("#ti_blo_p_fly_1 input:radio").filter(":checked").val();
    if (fop) {
        $("#ti_blo_p_fly_2").css("display", "block");
    }
    else {
        $("#ti_blo_p_fly_2").css("display", "none");
    }
