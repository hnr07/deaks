...
if ($params['BILL_PATH_TO_LOGO'])
{
	...
	$pdf->Image($params['BILL_PATH_TO_LOGO'], $width-100, $pdf->GetY(), 150);
}

$pdf->SetFont($fontFamily, 'B', $fontSize);

$pdf->SetX($pdf->GetX());
$pdf->Write(12, CSalePdf::prepareToPdf($params["SELLER_COMPANY_NAME"]));
$pdf->Ln();
...
if ($params["SELLER_COMPANY_ADDRESS"])
{
	...
	$pdf->SetX($pdf->GetX());
	$pdf->MultiCell(0, 12, CSalePdf::prepareToPdf($sellerAddr), 0, 'L');
}

if ($params["SELLER_COMPANY_PHONE"])
{
	$pdf->SetX($pdf->GetX());
	$pdf->Write(12, CSalePdf::prepareToPdf(Loc::getMessage('SALE_HPS_BILL_SELLER_COMPANY_PHONE', array('#PHONE#' => $params["SELLER_COMPANY_PHONE"]))));
	$pdf->Ln();
}
...
$pdf->Cell(280, 12,CSalePdf::prepareToPdf($params["SELLER_COMPANY_BANK_NAME"]),"L,T,R");
$pdf->Cell(50, 12, CSalePdf::prepareToPdf(Loc::getMessage('SALE_HPS_BILL_SELLER_BANK_BIK')),"T,R");	
$pdf->Cell(160, 12, CSalePdf::prepareToPdf($params['SELLER_COMPANY_BANK_BIC']),"T,R");
	$pdf->Ln();
	
$pdf->Cell(280, 12,CSalePdf::prepareToPdf($sellerBankCity),"L,R");	
$pdf->Cell(50, 12, '',"R");	
$pdf->Cell(160, 12, '',"R");
	$pdf->Ln();
	
$pdf->Cell(280, 12,'',"L,R");	
$pdf->Cell(50, 12, CSalePdf::prepareToPdf(Loc::getMessage('SALE_HPS_BILL_SELLER_ACC')),"R,T");	
$pdf->Cell(160, 13, CSalePdf::prepareToPdf($params['SELLER_COMPANY_BANK_ACCOUNT_CORR']),"R,T");
	$pdf->Ln();
	
$pdf->Cell(280, 12,CSalePdf::prepareToPdf(Loc::getMessage('SALE_HPS_BILL_SELLER_BANK_NAME')),"L,R");	
$pdf->Cell(50, 12, '',"R");	
$pdf->Cell(160, 12, '',"R");
	$pdf->Ln();
	
$pdf->Cell(50, 12, CSalePdf::prepareToPdf(Loc::getMessage('INN')),"L,T,R");	
$pdf->Cell(90, 12, CSalePdf::prepareToPdf($params['SELLER_COMPANY_INN']),"T,R");	
$pdf->Cell(50, 12, CSalePdf::prepareToPdf(Loc::getMessage('KPP')),"T,R");	
$pdf->Cell(90, 12, CSalePdf::prepareToPdf($params['SELLER_COMPANY_KPP']),"T,R");	
$pdf->Cell(50, 12, CSalePdf::prepareToPdf(Loc::getMessage('SALE_HPS_BILL_SELLER_ACC')),"T,R");	
$pdf->Cell(160, 12, CSalePdf::prepareToPdf($sellerRs),"T,R");
	$pdf->Ln();

$pdf->Cell(280, 12,CSalePdf::prepareToPdf($params["SELLER_COMPANY_NAME"]),"L,T,R");
$pdf->Cell(50, 12, CSalePdf::prepareToPdf(Loc::getMessage('VID_OP')),"T,R");	
$pdf->Cell(90, 12, '01',"T,R");
$pdf->Cell(50, 12, CSalePdf::prepareToPdf(Loc::getMessage('SROK_PLAT')),"T,R");
$pdf->Cell(20, 12, '',"T,R");
	$pdf->Ln();	
	
$pdf->Cell(280, 12,'',"L,R");
$pdf->Cell(50, 12, CSalePdf::prepareToPdf(Loc::getMessage('NAZ_PL')),"T,R");	
$pdf->Cell(90, 12, '',"T,R");
$pdf->Cell(50, 12, CSalePdf::prepareToPdf(Loc::getMessage('OCHER_PLAT')),"T,R");
$pdf->Cell(20, 12, '',"T,R");
	$pdf->Ln();
	
$pdf->Cell(280, 12,CSalePdf::prepareToPdf(Loc::getMessage('SALE_HPS_BILL_SELLER_NAME')),"L,R");
$pdf->Cell(50, 12, CSalePdf::prepareToPdf(Loc::getMessage('KOD')),"T,R");	
$pdf->Cell(90, 12, '',"T,R");
$pdf->Cell(50, 12, CSalePdf::prepareToPdf(Loc::getMessage('REZ_POLE')),"T,R");
$pdf->Cell(20, 12, '',"T,R");
	$pdf->Ln();
	
$pdf->Cell(490, 12, CSalePdf::prepareToPdf(Loc::getMessage('OPLATA_PO_ZAKAZU', array('#PAYMENT_NUM#' => $params["ACCOUNT_NUMBER"]))),"L,T,R");
	$pdf->Ln();
	
$pdf->Cell(490, 12, CSalePdf::prepareToPdf(Loc::getMessage('NAZNACHENIE_PLATEGA')),1);
	$pdf->Ln();
	$pdf->Ln();
$pdf->SetFont($fontFamily, 'B', $fontSize*2);
$pdf->Cell(0, 12, CSalePdf::prepareToPdf(Loc::getMessage('SALE_HPS_BILL_SELLER_TITLE', array('#PAYMENT_NUM#' => $params["ACCOUNT_NUMBER"], '#PAYMENT_DATE#' => $params["PAYMENT_DATE_INSERT"]))),0,0,"C");
	$pdf->Ln();
	$pdf->Ln();

$postavschik=Loc::getMessage('POSTAVSCHIK').$params["SELLER_COMPANY_NAME"].", ".Loc::getMessage('SALE_HPS_BILL_INN', array('#INN#' => $params['SELLER_COMPANY_INN'])).", ".Loc::getMessage('SALE_HPS_BILL_KPP', array('#KPP#' => $params['SELLER_COMPANY_KPP'])).", ".$params["SELLER_COMPANY_ADDRESS"].", ".Loc::getMessage('SALE_HPS_BILL_SELLER_COMPANY_PHONE', array('#PHONE#' => $params['SELLER_COMPANY_PHONE']));

$pdf->SetFont($fontFamily, '', $fontSize);

$pdf->Write(12,$postavschik);

	$pdf->Ln();
if ($params["BUYER_PERSON_COMPANY_NAME"])
{
	$pdf->Write(12, CSalePdf::prepareToPdf(Loc::getMessage('SALE_HPS_BILL_BUYER_NAME', array('#BUYER_NAME#' => $params["BUYER_PERSON_COMPANY_NAME"]))));
	if ($params["BUYER_PERSON_COMPANY_INN"])
		$pdf->Write(12, CSalePdf::prepareToPdf(Loc::getMessage('SALE_HPS_BILL_BUYER_PERSON_INN', array('#INN#' => $params["BUYER_PERSON_COMPANY_INN"]))));
	if ($params["BUYER_PERSON_COMPANY_ADDRESS"])
	{
		...
		$pdf->Write(12, CSalePdf::prepareToPdf(sprintf(", %s", $buyerAddr)));
	}
	if ($params["BUYER_PERSON_COMPANY_PHONE"])
		$pdf->Write(12, CSalePdf::prepareToPdf(sprintf(", %s", $params["BUYER_PERSON_COMPANY_PHONE"])));
	if ($params["BUYER_PERSON_COMPANY_FAX"])
		$pdf->Write(12, CSalePdf::prepareToPdf(sprintf(", %s", $params["BUYER_PERSON_COMPANY_FAX"])));
	if ($params["BUYER_PERSON_COMPANY_NAME_CONTACT"])
		$pdf->Write(12, CSalePdf::prepareToPdf(sprintf(", %s", $params["BUYER_PERSON_COMPANY_NAME_CONTACT"])));
	$pdf->Ln();
}
...
for ($n = 1; $n <= $rowsCnt; $n++)
{
	...
	for ($l = 0; $pdf->GetStringWidth($text) > 0; $l++)
	{
		...
		{
			if (is_null($arCells[$n][2]))
				$pdf->Cell($arRowsWidth_tmp[6], 12, $string, 0, 0, 'R');
			else if ($vat > 0)
				$pdf->Cell($arRowsWidth_tmp[6], 12, ($l == 0) ? $arCells[$n][6] : '', 0, 0, 'R');
		}
		if ($l == 0)
			$x6 = $pdf->GetX();

		if (!is_null($arCells[$n][7]))
			$pdf->Cell($arRowsWidth_tmp[7], 12, ($l == 0) ? $arCells[$n][7] : '', 0, 0, 'R');
		if ($l == 0)
			$x7 = $pdf->GetX();
...
$pdf->SetFont($fontFamily, '', $fontSize);
$pdf->Write(12, CSalePdf::prepareToPdf(Loc::getMessage(
...
{
	$pdf->Write(12, CSalePdf::prepareToPdf(Number2Word_Rus($payment->getSum())));
}
else
{
	$pdf->Write(12, CSalePdf::prepareToPdf(SaleFormatCurrency(
		$payment->getSum(),
...
{
	//$pdf->Write(13, CSalePdf::prepareToPdf(Loc::getMessage('SALE_HPS_BILL_COND_COMM')));
	//$pdf->Ln();
...
if ($params["SELLER_COMPANY_DIRECTOR_POSITION"])
{
	...
	$pdf->MultiCell(150, 12, $sellerDirPos, 0, 'L');
	...
	if ($params["SELLER_COMPANY_DIRECTOR_NAME"])
		$pdf->Write(12, CSalePdf::prepareToPdf('('.$params["SELLER_COMPANY_DIRECTOR_NAME"].')'));
	$pdf->Ln();
...
if ($params["SELLER_COMPANY_ACCOUNTANT_POSITION"])
{
	...
	if ($isAccSign && $pdf->GetStringWidth($sellerAccPos) <= 160)
		$pdf->SetY($pdf->GetY() + min($signHeight, 30) - 15);
	$pdf->MultiCell(150, 12, $sellerAccPos, 0, 'L');
	$pdf->SetXY($margin['left'] + 150, $pdf->GetY() - 15);
	...
	if ($params["SELLER_COMPANY_ACCOUNTANT_NAME"])
		$pdf->Write(12, CSalePdf::prepareToPdf('('.$params["SELLER_COMPANY_ACCOUNTANT_NAME"].')'));
	$pdf->Ln();
