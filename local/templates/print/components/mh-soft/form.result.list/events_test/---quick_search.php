...
if($tqs!="") {
	...
	if($answer_id) 
	{
		...
		$query_answer_id= mysql_query($za_answer_id); 
		$rows_answer_id=mysql_num_rows($query_answer_id);
		
		if($rows_answer_id) 
		{
			for($i=0;$i<$rows_answer_id;$i++) 
			{
			$pa_answer_id=mysql_fetch_array($query_answer_id);
	...
	$query_id= mysql_query($za_id); 
	$rows_id=mysql_num_rows($query_id);
	if($rows_id) 
	{
		for($i=0;$i<$rows_id;$i++) 
		{
			$pa_id=mysql_fetch_array($query_id);
