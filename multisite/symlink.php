		...
		if ($_POST['create'])
		{
		   ...
		   elseif (is_dir($full_path))
		   {
			  if (is_dir($full_path."/bitrix"))
			  {
				 ...
				 {
					if(symlink($path."/upload",$_SERVER['DOCUMENT_ROOT']."/upload") && symlink($path."/local",$_SERVER['DOCUMENT_ROOT']."/local"))
					   echo "Символические ссылки удачно созданы";
					else
					   $strError = 'Не удалось создать ссылку на папку upload и local, обратитесь к администратору сервера';
				 }
				 else
