...
if(CModule::IncludeModule("form")){ 
	...
	$ar_Answer = CFormResult::GetDataByID(
		$RESULT_ID, 
		array('name','family','email','email_2','em_bd','billingdate','claimdate','history_status','lang_id','code_m'),  // массив символьных кодов необходимых вопросов
		$ar_Result, 
		$ar_Answer2);  
	...
	if(!($ar_Answer['billingdate'][0]['USER_TEXT'])) {	                             // Если нет даты выставления счёта
		CFormResult::SetField( $RESULT_ID, "billingdate", array ($ar_Answer['billingdate'][0]['ANSWER_ID'] => $ar_Answer['claimdate'][0]['USER_TEXT'])); // Дату поступления заявки вносим в результат, как дату выставления счёта
		} 
...
if($CURRENT_STATUS_ID <>$PREV_STATUS_ID) {
	...
	@include $_SERVER["DOCUMENT_ROOT"]."/com/registration_event/".$code_m."/mail_status/lang/".$lang_id."/status_ok.php"; //шаблон письма на языке регистрации заявки
