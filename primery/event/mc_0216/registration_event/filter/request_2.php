...
$status_ok=4;  //id статуса Подтверждена
$status_nepr=5;  //id статуса Ожидает промоушен
$status_opl=6;  //id статуса Ожидает оплаты
$status_nopl=7;  //id статуса Истёк срок оплаты
$status_rz=8;  //id статуса Резерв
$status_del=9;  //id статуса Отменена

$ccws1=new ccws_event();
//global $href;
...
//array('chk','family','name','kem_priglashen_chk','kem_priglashen_family','kem_priglashen_name','sum_debt','money_2_calc','currency','promotion_1','fly_1','fly_2','hotel','nomer','p_hotel','d_leader_ship','s_leader_ship','hotel_ls','nomer_ls','status','comments_admin','mesto','claimdate'), 	
	...
	$ar_Ans2);
					...
					default: $ar_xml_var["Condition"]=5;       //  Поступила
					}
					//echo $status_del;
					//присваиваем код формы оплаты
						...
						if($ar_Ans['status'][0]['ANSWER_VALUE']=="member") 
						{
						//print_r($ar_Ans['chk'][0]["USER_TEXT"])."<-1";
						$ar_xml_var["PayerId"]=trim($ar_Ans['chk'][0]["USER_TEXT"]); 
						$ar_xml_var["Payment"]=2;
						$ar_xml_var["Amount"]=0;
						$ar_xml_var["SummitAmount"]=0;
						}
						
						if($ar_Ans['status'][0]['ANSWER_VALUE']=="guest")  {
					...
					//присваиваем Стоимость в базовой валюте	
						...
						$ar_xml_var["PayerId"]=trim($ar_Ans['kem_priglashen_chk'][0]["USER_TEXT"]); //  плательщик - приглашающий № ЧК
				...
				//	else $errors_request[]=$code_m.": заявка № ".$ar_xml_var["Id"]." Ошибка: Нет или ошибка формата даты поступления заявки(ClaimDate - claimdate)";// если дата поступления заявки не указана или неформат запишем ошибку в массив
					...
					$ar_xml_var["BillingDate"]=$ccws1->Atom_date($ar_Ans['billingdate'][0]["USER_TEXT"]);
					$ar_xml_var["ClaimDate"]=$ar_xml_var["BillingDate"];
							...
							array( //Основной метод отправляющий заявку на сервис
								...
								'Workflow'  => array(
									array(
										...
										"ClaimDate"=>$ccws1->Atom_date($ar_xml_var["ClaimDate"]), //ИЗМЕНЕНИЮ НЕ ПОДЛЕЖИТ! Дата подачи заявки, Atom_date переводит в нужный формат
										"BillingDate"=>$ccws1->Atom_date($ar_xml_var["BillingDate"]), //ИЗМЕНЕНИЮ НЕ ПОДЛЕЖИТ! Дата выставления счета, Atom_date переводит в нужный формат
