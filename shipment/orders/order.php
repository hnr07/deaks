...
//--------------настройки конец-------------------//
 ...
 // массив полей
$available = array('WHAT_CARRING'=>'Что везём', 'QUANTITY'=>'', 'TO_ADR'=>'Куда', 'TO_ADR_COORDS'=>'', 'FROM_ADR_COORDS'=>'', 'DISTANCE'=>'', 'PRICE'=>'Груз', 'COUNT_CAR'=>'Сколько авто', 'PRICE_DELIVERY'=>'Доставка', 'SUM'=>'Сумма', 'WHEN'=>'Когда', 'PERIOD'=>'', 'FIO'=>'Имя', 'PHONE'=>'Телефон(от 6 цифр)', 'FIO_2'=>'Имя', 'PHONE_2'=>'Телефон', 'ID_PAY'=>'Оплата'/*,'TIME_1'=>'','TIME_2'=>''*/, 'NOTE'=>'Комментарий');
...
<input type="hidden" id="step_q" value="<?=$step_q?>">
...
<input type="hidden" id="control_adr" value="">
	<form id="form_order" name="form_order">
		...
		<?}?>
		<div class="err"></div>
		<div class="loader"><img src="/images/shipment/loader_35.gif" /></div>
	...
	*/
	...
	$('#form_order').on("submit", function(){
		if(erpo()==true) {
			...
			var url="/shipment/orders/add_order.php";
			$.ajax({  
				type: "POST",
				url: url,  
				data: data, 
...
*/
	...
	$('.datetimepicker[name=WHEN]').datetimepicker({
		dayOfWeekStart : 1, // начало недели 1-понедельник
		...
		disabledWeekDays:[0,6], // неактивные дни недели 0-воскресенье, 6-суббота
